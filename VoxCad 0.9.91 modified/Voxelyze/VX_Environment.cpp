...
void CVX_Environment::WriteXML(CXML_Rip* pXML)
{
	pXML->DownLevel("Environment");
		...
		pXML->Element("Temp2Amplitude", Temp2Amplitude);
...
bool CVX_Environment::ReadXML(CXML_Rip* pXML, std::string* RetMessage) //pXML pointer to an "Environment" element
{
	...
	if (pXML->FindElement("Thermal")){
		...
		if (!pXML->FindLoadElement("Temp2Amplitude", &Temp2Amplitude)) Temp2Amplitude = 0;
...
void CVX_Environment::UpdateCurTemp(vfloat time) //!!NOTE CurTemp only used here, only material temps relevant
{
	if (VaryTempEnabled){
		...
		for (int i = 0; i<(int)pObj->GetNumMaterials(); i++){ //now update the individual temperatures of each material (they can each have a different temperature)
			vfloat TrueTempAmp = pObj->GetBaseMat(i)->GetTempNum()==1 ? TempAmplitude : Temp2Amplitude; //!!NOTE Chose correct temp
			pObj->GetBaseMat(i)->SetCurMatTemp(TempBase + TrueTempAmp*sin((2*3.1415926f/TempPeriod) * time + pObj->GetBaseMat(i)->GetMatTempPhase()));	//and update each one
		}
	}
	else {
		CurTemp = TempBase + TempAmplitude;
		for (int i = 0; i<pObj->GetNumMaterials(); i++){ //for each material...
			vfloat TrueTempAmp = pObj->GetBaseMat(i)->GetTempNum()==1 ? TempAmplitude : Temp2Amplitude;
			pObj->GetBaseMat(i)->SetCurMatTemp(TempBase + TrueTempAmp); //...update each one
		}
	}
